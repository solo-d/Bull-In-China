...
public class WinScriptcopy : MonoBehaviour
{
    ...
    public GameObject TryBut; //tryagain button
    public GameObject TimeCheck;
    public GameObject MainCamera;
    public bool checkWin;
    public GameObject[] levelObject;
    public string[] level = { "Plate", "Vase", "Mug", "Bowl" };
    public string[] outlineArray = { "O-Plate", "O-Vase", "O-Mug", "O-Bowl" };
    ...
    public int round = 1; //keep track on how many wins
    ...
    {
        levelpicker = Random.Range(0, level.Length - 1); //generate number between 0 and 3 inclusive
        ceramic = level[levelpicker]; //stores string name - vase
        outline = outlineArray[levelpicker];
        checkLose = false;
        checkWin = false;
        levelObject = GameObject.FindGameObjectsWithTag(level[levelpicker]);

        Debug.Log(levelObject);
        Debug.Log("Level Tag:" + ceramic); //string name returned
        Debug.Log("Outline: " + outline);

        for (int i = 0; i <= level.Length - 1; i++) //then set the objects with the tag to become Active as it starts as hidden
        { 
            if (ceramic != level[i]) //vase is not equal itself (aka 1)
            {
                var gameObjectArray = GameObject.FindGameObjectsWithTag(level[i]); //get other ceramics into an array
        ...
        for (int i = 0; i <= outlineArray.Length - 1; i++)
        {
            if (outline != outlineArray[i])
            {
                var gameObjectArray = GameObject.FindGameObjectsWithTag(outlineArray[i]);
    ...
    void fixedIt()
    {
        ...
        {
            round = round++; //if you win, add 1 to the scenetracker variable
            //Show next level button
        }

       else if (round == 2 && TimeCheck.GetComponent<CountDownTimer>().CurrentTime >= 40) //Round 2 - Bonus Button, Title Screen
        {
            round = round++; // int becomes 2
